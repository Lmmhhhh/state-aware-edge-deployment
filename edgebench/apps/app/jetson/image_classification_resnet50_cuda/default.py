...
def main():
    ...
    print("[workload=image_classification][model=resnet50][mode=single][device=jetson][accel=cuda] start")

    # model
    weights = ResNet50_Weights.DEFAULT
    model = resnet50(weights=weights).eval().to("cuda")
    ...
    print(
        f"[workload=image_classification][model=resnet50][mode=single][device=jetson][accel=cuda] "
