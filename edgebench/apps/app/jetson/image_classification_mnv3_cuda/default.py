...
def main():
    ...
    print("[workload=image_classification][model=mobilenetv3_large][mode=single][device=jetson][accel=cuda] start")

    # model
    weights = MobileNet_V3_Large_Weights.DEFAULT
    model = mobilenet_v3_large(weights=weights).eval().to("cuda")
    preprocess = weights.transforms()

    # choose 1 image
    img_path = pick_fixed_image(img_dir)
    x = preprocess(Image.open(img_path).convert("RGB")).unsqueeze(0).to("cuda")

    # warm-up (to reduce first-run jitter)
    ...
    print(
        f"[workload=image_classification][model=mobilenetv3_large][mode=single][device=jetson][accel=cuda] "
