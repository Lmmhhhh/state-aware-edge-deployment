...
def main():
    ...
    print("[workload=object_detection][model=yolov8s][device=jetson][mode=cuda] start")
    t0 = time.perf_counter()

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available, but this script is for Jetson CUDA mode.")
    ...
    print(
        "[workload=object_detection][model=yolov8s][device=jetson][mode=cuda] "
